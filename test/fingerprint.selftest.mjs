...
import { extractFingerprints } from '../web/js/peer.js';
...
// The decision the peer makes: overlap ⇒ authentic, no overlap ⇒ MITM.
...
const overlap = peerReal.some((f) => relayed.includes(f));
ok(overlap === false, 'a swapped (MitM) fingerprint shares nothing with the real one → detected');

const honestRelayed = extractFingerprints(sdpA);
ok(peerReal.some((f) => honestRelayed.includes(f)) === true, 'an untampered fingerprint matches → media path trusted');
